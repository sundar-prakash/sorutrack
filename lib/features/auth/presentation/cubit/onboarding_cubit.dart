...
class OnboardingCubit extends Cubit<OnboardingState> {
  ...
  OnboardingCubit(this._saveUserProfile, this._geminiKeyService)
      : super(const OnboardingState());

  void updateName(String name) =>
      emit(state.copyWith(name: name, error: null));
  void updateDateOfBirth(DateTime dob) =>
      emit(state.copyWith(dateOfBirth: dob, error: null));
  ...
  bool nextStep() {
    ...
    int nextStepIndex = state.currentStep + 1;
    if (state.currentStep == 3 && state.goal == GoalType.maintain) {
      nextStepIndex = 5; // Skip targets
    }
    emit(state.copyWith(currentStep: nextStepIndex, error: null));
    return true;
  }

  void previousStep() {
    int prevStepIndex = state.currentStep - 1;
    if (state.currentStep == 5 && state.goal == GoalType.maintain) {
      prevStepIndex = 3;
    }
    emit(state.copyWith(currentStep: prevStepIndex, error: null));
  }
  ...
  String? _validateCurrentStep() {
    switch (state.currentStep) {
      ...
      case 1:
        ...
        if (state.weight < 20 || state.weight > 500) {
      ...
      case 4:
        ...
        if (state.goal == GoalType.gainMuscle &&
            state.targetWeight <= state.weight) {
          return 'Target weight must be greater than current weight to gain muscle';
        }
        break;
      // Step 6 (Gemini key): no hard block — handled by dialog in the screen
    }
    return null;
  }
  ...
  /// Tests the Gemini key that is currently in state. Returns the result.
  Future<ApiKeyValidationResult> validateAndTestGeminiKey() async {
    if (state.geminiApiKey.isEmpty) return ApiKeyValidationResult.invalid;
    return await _geminiKeyService.validateKey(state.geminiApiKey);
  }
  ...
  Future<void> submit() async {
    ...
    final profile = UserProfile(
      ...
      mealReminderAfternoon:
          state.mealReminderAfternoon ?? DateTime(2026, 1, 1, 13),
      mealReminderEvening:
          state.mealReminderEvening ?? DateTime(2026, 1, 1, 20),
    ...
    if (state.geminiApiKey.isNotEmpty) {
      try {
        ...
        emit(state.copyWith(
            isSubmitting: false,
            error: 'Failed to save API Key: ${e.toString()}'));
        return;
      }
    }
    ...
    result.fold(
      ...
      (_) async {
        // ✅ Persist the onboarding completion flag so the router redirect works
